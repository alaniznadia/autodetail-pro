  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto max-w-[560px] px-4 py-20 text-center">
        <h1 className="text-[28px] font-medium">Tu carrito está vacío</h1>
        <p className="mt-2 text-[18px] text-foreground/78">Todavía no agregaste productos.</p>
        <Link
          href="/catalogo"
          className="store-frame mt-8 inline-block border-accent px-7 py-3 text-[18px] text-accent hover:bg-accent/10"
        >
          Ver catálogo
        </Link>
      </div>
  ...
  return (
    <div className="mx-auto flex max-w-[1100px] flex-col gap-9 px-4 pb-14 sm:px-8 lg:flex-row">
      <div className="min-w-0 flex-1">
        <h1 className="mb-6 text-[28px] font-medium tracking-[-0.02em]">Tu carrito</h1>
          ...
          {items.map((item) => (
            <li key={item.variantId} className="flex items-center gap-4 p-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[17.5px] text-foreground/70">{money(Number(item.price))} c/u</p>
                <button
                  type="button"
                  onClick={() => removeItem(item.variantId)}
                  aria-label={`Quitar ${item.productName} del carrito`}
                  className="mt-1 text-[16px] text-foreground/70 underline underline-offset-2 hover:text-foreground"
                >
                  Quitar
                </button>
              </div>
              <div className="store-frame flex h-10 items-center border-border">
                <button
                  ...
                  className="h-full w-9 text-[20px]"
                >
                  −
                </button>
                <span className="w-7 text-center text-[18px]" aria-live="polite">{item.quantity}</span>
                <button
                  type="button"
                  onClick={() => updateQuantity(item.variantId, item.quantity + 1)}
              ...
              <p className="w-20 shrink-0 text-right text-[19px]">
      ...
      <aside className="store-frame flex w-full shrink-0 flex-col gap-4 self-start border-border p-5 lg:sticky lg:top-[88px] lg:w-[320px]">
        <form onSubmit={applyCoupon} className="flex flex-col gap-2">
          <label htmlFor="coupon" className="text-[15.5px] uppercase tracking-[0.14em] text-foreground/62">
            Cupón de descuento
          </label>
          <div className="flex gap-2">
            ...
            <button
              ...
              className="store-frame h-10 shrink-0 border-border px-4 text-[18px] hover:bg-foreground/5 disabled:opacity-50"
            >
              Aplicar
            </button>
          </div>
          {couponState.status === "error" ? (
            <p className="text-[16px] text-red-400">{couponState.message}</p>
          ) : null}
          {couponState.status === "ok" ? (
            <p className="text-[16px] text-accent">Cupón aplicado: −{money(couponState.discount)}</p>
          ) : null}
        </form>
        ...
        <div className="flex flex-col gap-2 text-[18px]">
          ...
          <p className="text-[16px] text-foreground/70">El envío se calcula en el checkout.</p>
        </div>
        ...
        <div className="flex items-baseline justify-between">
          ...
          <span className="text-[24px] tracking-[-0.02em]">{money(total)}</span>
        </div>
        ...
        <Link
          href="/checkout"
          className="store-frame mt-2 flex h-[46px] items-center justify-center border-accent text-[18px] text-accent hover:bg-accent/10"
        >
          Finalizar compra
        </Link>

        {loyaltyEnabled && pointsEarned > 0 ? (
          <p className="text-[16px] text-foreground/70">Sumás {pointsEarned} puntos con esta compra.</p>
        ) : null}
      </aside>
    </div>
  );
